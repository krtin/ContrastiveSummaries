...
prob_smc_data = pkl.load(open(config.gtp_smc_probfile, 'rb'))
...
if(len(smc_data)!=len(prob_smc_data)):
    ...
    smc_data['oov_words'] = prob_smc_data['gtp_target']==prob_smc_data['gtp_output']
    smc_data['gtp_log_prob'] = prob_smc_data['log_prob']
    smc_data['gtp_avg_log_prob'] = prob_smc_data['avg_log_prob']
...
probfiles = [f for f in os.listdir(config.gtp_smic_probdir) if os.path.isfile(os.path.join(config.gtp_smic_probdir, f))]
...
for filename in probfiles:
    ...
    if(len(filename_parts)==3):
        ...
        if(is_number(filename_parts[1]) and is_number(filename_parts[2])):
            ...
            prob_smic_data[start] = pkl.load(open(os.path.join(config.gtp_smic_probdir, filename), 'rb'))
...
prob_added_len = smic_data['gtp_log_prob'].count()
