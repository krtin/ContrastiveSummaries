...
with open('../summarization_models/namas_trained/probs/gigaword_smic.txt', 'r') as f:
    smic_probs = f.read().split('\n')
    if(smic_probs[-1]==''):
        smic_probs.pop()

with open('../summarization_models/namas_trained/probs/gigaword_smc.txt', 'r') as f:
    smc_probs = f.read().split('\n')
    if(smc_probs[-1]==''):
        smc_probs.pop()
...
with open('data/giga_nov_classes.txt', 'r') as f:
    class_data = f.read().split('\n')
    if(class_data[-1]==''):
        class_data.pop()

#print(smc_probs)
smc_data = pd.read_csv('data/gigaword_corpus.csv', dtype={'idx': np.int64, 'smc': object, 'source': object}, usecols=['idx', 'smc', 'source'])
smc_data['smc_prob'] = np.array(smc_probs).astype(np.float64)
...
smc_data['nov1_classes'] = np.array(class_data).astype(np.int32)
...
smic_data['smic_prob'] = np.array(smic_probs).astype(np.float64)
