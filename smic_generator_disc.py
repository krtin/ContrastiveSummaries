...
#nlp = StanfordCoreNLP('http://dp-gpu4.cs.mcgill.ca:9000/')
#nlp = StanfordCoreNLP('http://jc-gpu1.cs.mcgill.ca:9000/')
rules = 'rules.json'
...
class smic_generator(object):
    ...
    def generate(self,source, gold, smc, smcscore, sourceid, judgeid, debugrule=0):
        ...
        smc_short = smc.split(" ")
        smc_short = " ".join(smc_short[0:min(len(smc_short),100)])
        sys_parse = nlp.annotate(smc_short, properties={
        ...
        if(type(source)==str):
            source_short = source.split(" ")
        else:
            return []

        source_short = " ".join(source_short[0:min(len(source_short),400)])
        source_parse = nlp.annotate(source_short, properties={
            'annotators': 'tokenize,ssplit,pos,depparse,parse',
            'outputFormat': 'json'
        })
        ...
        total_coverage = []
        smc_rules_dist = [0.69,0.07,0.18,0.06]
        total_smc_smics = 25
        smc_rules_count = [16, 2, 5, 2]
        source_rules_dist = [0.63,0.08,0.19,0.1]
        total_source_smics = 25
        source_rules_count = [15, 2, 5, 3]
            ...
            if(rule["id"]==debugrule or debugrule==0 ):
                ...
                if rule['from']=='SMC':
                    ...
                    for subset in subsets:
                        ...
                        approx_sents += (float(len(submatched)*(len(submatched)-1))/2.)
                    ...
                    for subset in subsets:
                        ...
                        #matched will contain list of matches for each subset
                        matched.append(submatched)
                        approx_sents_gold += (len(submatched)*len(submatched_gold))
                        #repeat for matches in gold standard
                        matched_gold.append(submatched_gold)

                #print("For rule %d gold matches %d matches %d " % (rule['id'], len(matched_gold), len(matched)))
                ...
                if rule['from']=='SMC':
                    ...
                    for match in matched:
                        if(len(match)>0 and approx_sents>0):

                            sent_limit = np.ceil((float(len(match)*(len(match)-1))/2.)/float(approx_sents)*source_rules_count[rule['id']-1])


                            #function which generates the sentences
                            sentence, coverage = self.generateSentence(dtrees, [], match, [], gold, int(rule['hardmatch']), smcscore, sourceid, smcLMScore, rule["id"], smc, judgeid, rule['from'], total_coverage, sent_limit)
                    ...
                    for match, match_gold in zip(matched, matched_gold):
                        ...
                        if(len(match)>0 and len(match_gold)>0 and approx_sents_gold>0):
                            #ceil will ensure no value is zero
                            sent_limit = np.ceil(float(len(match)*len(match_gold))/float(approx_sents_gold)*source_rules_count[rule['id']-9])


                            #function which generates the sentences
                            sentence, coverage = self.generateSentence(dtrees, dtrees_secondary, match, match_gold, gold, int(rule['hardmatch']), smcscore, sourceid, smcLMScore, rule["id"], smc, judgeid, rule['from'], total_coverage, sent_limit)
                            if(len(coverage)!=0):
                                total_coverage.extend(coverage)
                            sentences.extend(sentence)
                            count += 1
                            #print(count)
                #print("sentences %d the limit was %d"% (len(sentences), sent_limit))
    ...
    def generateSentence(self, dtrees, dtrees_gold, matches, matches_gold, gold, hardmatch, smcscore, sourceid, smcLMScore, ruleid, smc, judgeid, rulefrom, cov_check, sent_limit):
        ...
        np.random.seed(496)
        listof_mathces = list(matched)
        np.random.shuffle(listof_mathces)
        for match in listof_mathces:

            #if(len(sentences)==sent_limit):
            #    break
            ...
            if(ruleid==3 or ruleid==7 or ruleid==11):
                ...
                if(tree1_node_pos!=tree2_node_pos):
                    ...
                    #print(tree1_word_list, tree2_word_list, tree1_node_pos, tree2_node_pos, tree1_node.tag, tree2_node.tag)
                    ...
                    if(len(tree1_word_list)==0 or len(tree2_word_list)==0):
                            ...
                            for tree1_word in tree1_word_list:
                                ...
                                if(rulefrom == 'SMC'):
                                    ...
                                    if(tuple_tosearch in match_coverage or tuple(reversed(tuple_tosearch)) in match_coverage):
                                        ...
                                        continue
                                if(rulefrom == 'SOURCE' or rulefrom == 'GOLD'):
                                    if(tree2_node.tag.lower() in cov_check or tuple_tosearch in match_coverage):
                                        continue
                        ...
                        if((rulefrom == 'SOURCE' or rulefrom == 'GOLD') and len(tree1_word_list)>1):
                            tree1_word_list = set(list(tree1_word_list)[0:1])
